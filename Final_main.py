...
# In[] Generate AUG train.txt val.txt test.txt 
...
for i_class in target_classes:
    ...
    if len(i_image_path_list) > 5000:
        # break
        temp = random.choices(i_image_path_list, k = 5000)
        i_image_path_list = temp
    elif len(i_image_path_list) < 5000:
        temp = random.choices(i_image_path_list, k = aug_max_number - len(i_image_path_list))
        i_image_path_list += temp
...
with open(os.path.join(ROOT, 'train_bal5000.txt'), 'w') as file:
        for row in train_list:
            file.write(row + '\n')

with open(os.path.join(ROOT, 'val_bal5000.txt'), 'w') as file:
        for row in val_list:
            file.write(row + '\n')

with open(os.path.join(ROOT, 'test_bal5000.txt'), 'w') as file:
        for row in test_list:
            file.write(row + '\n')
...
train_txt = 'data/train_bal5000.txt'
val_txt = 'data/val_bal5000.txt'
test_txt = 'data/test_bal5000.txt'
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), f'model_weights/{model_name}_model_{img_size}_{best_valid_loss}.pt')
...
# In[] plot loss
    ...
    plt.savefig(f'Training Curve of {model_name}.png')
    plt.close()
...
# In[] plot acc
    ...
    plt.savefig(f'Accuracy of {model_name}.png')
    plt.close()
